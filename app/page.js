...
//   // Use a ref to track the absolute end time to prevent background lag
...
//     expectedEndTime.current = null; // Clear the ref for the new session
...
//   // Absolute Timer Logic
//   useEffect(() => {
//     let interval;
    
//     if (isRunning && timeLeft > 0) {
//       // Ensure the target end time is set using the system clock
//       if (!expectedEndTime.current) {
//         expectedEndTime.current = Date.now() + timeLeft * 1000;
//       }

//       interval = setInterval(() => {
//         const now = Date.now();
//         const distance = expectedEndTime.current - now;
//         const remainingSeconds = Math.max(0, Math.round(distance / 1000));

//         setTimeLeft(remainingSeconds);
        
//         // Keep your total study time tracking
//         if (mode === "study") {
...
//         // Handle session end within the interval based on absolute time
...
//   }, [isRunning, mode, handleSessionEnd, timeLeft]); 

//   // Toggle Timer with absolute time adjustments
//   const toggleTimer = useCallback(() => {
//     setIsRunning((prev) => {
//       if (!prev) {
//         // Starting/resuming: calculate target time using the system clock
//         expectedEndTime.current = Date.now() + timeLeft * 1000;
//       } else {
//         // Pausing: clear the target time so it recalculates perfectly on resume
...
//     setMode("study");
//     setTimeLeft(studyDuration * 60);
//     expectedEndTime.current = null; // Clear the absolute time
...
//       // If we are undoing a reset and it was running, recalculate end time
//       if (previousState.isRunning) {
//         expectedEndTime.current = Date.now() + previousState.timeLeft * 1000;
//       } else {
//         expectedEndTime.current = null;
//       }
//     }
...
const Pomodoro = () => {
  return (
    <div className='flex items-center justify-center h-screen text-2xl'>G4Gate Timer = Study, focus, Learn</div>
  )
}
