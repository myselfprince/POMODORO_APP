...
export default function ObsOverlay() {
  ...
  // New Tracking State
  const [accumulatedStudySeconds, setAccumulatedStudySeconds] = useState(0);
      ...
      if (savedState) {
        ...
        if (isNewDay) {
          ...
          saveState({
            ...
            accumulatedStudySeconds: 0, sessionsCompleted: 0, pauseTimeLeft: initialSeconds, targetTime: null
          });
        } else {
          setPhase(parsed.phase || "study");
          setCurrentDuration(parsed.currentDuration || 60);
          setIsRunning(parsed.isRunning || false);
          setAccumulatedStudySeconds(parsed.accumulatedStudySeconds || 0);
        ...
        saveState({
          ...
          volume: 0.5, accumulatedStudySeconds: 0, sessionsCompleted: 0
        });
      }
    }
  }, []);
  ...
  const tick = useCallback(() => {
    ...
    if (logicalDay !== currentLogicalDayRef.current) {
        ...
        saveState({
            ...
            accumulatedStudySeconds: 0, sessionsCompleted: 0
        });
        return;
    }
    ...
    if (remaining <= 0) {
      ...
      let newAccum = accumulatedStudySeconds;

      if (phase === "study") {
        newAccum += (currentDuration * 60);
        newSessions += 1;
      ...
      setAccumulatedStudySeconds(newAccum);
      ...
      saveState({
        ...
        pauseTimeLeft: nextDurationSeconds, accumulatedStudySeconds: newAccum,
        sessionsCompleted: newSessions
      });
    } else {
      requestRef.current = requestAnimationFrame(tick);
    }
  }, [isRunning, phase, currentDuration, defaultStudyStr, defaultBreakStr, longBreakStr, accumulatedStudySeconds, sessionsCompleted]);
  ...
  const skipPhase = () => {
    ...
    if (phase === "study") {
      // Add only the time we actually studied before skipping
      newAccum += Math.max(0, (currentDuration * 60) - timeLeft);
      newSessions += 1;
    ...
    saveState({
      ...
      targetTime: newTargetTime, accumulatedStudySeconds: newAccum, sessionsCompleted: newSessions
    });
  };
  ...
  const handleStartCustom = (targetPhase, minutesStr) => {
    ...
    saveState({ phase: targetPhase, currentDuration: validMinutes, pauseTimeLeft: seconds, targetTime: newTargetTime });
  };
  ...
  // --- TIMELINE LOGIC ---
  // Real-time calculation of total studied seconds (past accumulated + ongoing session)
  const displayStudySeconds = accumulatedStudySeconds + (phase === "study" ? Math.max(0, (currentDuration * 60) - timeLeft) : 0);
  ...
  let remainingDisplay = displayStudySeconds;
